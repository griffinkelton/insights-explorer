...
- **GA4 OAuth (server-owned, PKCE S256):** `POST /api/v1/ga4/connect` (cryptographic state + PKCE verifier, 10-min TTL transaction record, HttpOnly transaction cookie, S256 auth URL) → `GET /api/v1/ga4/callback` (one-time state consumption, `compare_digest` txn-cookie binding, server-side code exchange with stored verifier + allowlisted redirect URI, **grant-scope verification**, encrypted server-side tokens, **app-session rotation**, 303 → `/auth/{ga4|drive}/callback?status=…`). Disconnect revokes. Status reflects the connection.
- **GA4 first pull:** locked D3 report — 5 canonical metrics (`sessions`/`totalUsers`/`engagedSessions`/`engagementRate`/`bounceRate`) × `date`, 90 complete days, daily grain, `returnPropertyQuota`; generic offset pagination (10k pages) **proven via mocks** (~90-row production reality); quota recorded from successful responses only; provenance object (safe aggregate); full typed taxonomy (`ga4_connection_required`/409, `ga4_reconnect_required`/401, `ga4_access_denied`/403, `ga4_property_unavailable`/404, `ga4_invalid_report`/422, `ga4_rate_limited`/429, **non-retryable `ga4_quota_exhausted`**/429, `ga4_provider_unavailable`/503, `ga4_timeout`/504); metric-status policy via `build_contract_metrics` (rows 3–5 `unavailable`, never numeric).
- **Drive Picker-first (download-and-ingest only, no upload):** separate `drive.file` consent; `POST /api/v1/drive/picker-token` (JIT, `Cache-Control: no-store` + `Pragma: no-cache`, real `expires_at` + `app_id`, token memory-only in `DrivePickerDialog`, never revoked on close); `POST /api/v1/drive/download` accepts **`{ request_id, file_id }` only** — stale/duplicate `request_id` → typed non-retryable error, server re-fetches metadata (`trashed` + `capabilities(canDownload)`), declared + actual-byte caps (100 MB), disk-backed `NamedTemporaryFile` with sanitized suffix, worker-thread execution, temp artifact deleted in `finally` (cancellation-safe), `workspace_export_required` for Google-native files, old dataset preserved on failure.
- **Backend infra:** `api/stores/oauth_store.py` (TTL transactions), settings (`GA4_*`/`DRIVE_*` + `drive_download_timeout_seconds`), guard allowlist + `.env.example`, routers wired in `api/main.py`, same-origin CSRF on unsafe requests.
- **Frontend:** `api.ts` error map (`ga4_not_configured` fix + full ga4/drive codes) + endpoints; store real members (`connectGA4`, `handleGA4Callback`, `connectDrive`, `openDrivePicker`, `downloadFromDrive`, `refreshConnections`) + `clearError`; **`ErrorBanner`** surfaces typed errors on the empty state; `DrivePickerDialog` (memory-only token, CSV/XLSX filter, Sheets labelled unsupported); `OAuthCallbackPage` + GA4/Drive callback routes; MSW handlers + fixtures + 8 store tests; CI e2e job now runs both Playwright gates.
- **Review-fix round (same commit):** callback cookies set on the returned `RedirectResponse` (injected `Response` cookies were discarded); `anyio.to_thread.run_sync` lambda for 4.x; raw provider exceptions classified via `classify_ga4_error`; `retryable` included in typed errors; cancel-during-transfer leaves no orphan artifact and preserves the prior dataset; drive denial routes to the `/auth/drive/callback` page via state peek.
- **D4 residual (explicitly pending, never falsely closed):** live opt-in Google smoke + post-OAuth property probe need owner sandbox credentials (`E2E_REAL_GOOGLE=1`, never CI). The no-credential e2e gate (`test_phase5_wiring_smoke.py`) closes the wiring; the 12-row Drive matrix stays opt-in.
- **Docs synced:** `phase-5-ga4-drive.md` (banner → DONE, exit criteria checked, gate closed), specs README (row + next-spec → Phase 6), master-plan §9 STATUS → DONE + §10 banner refreshed, DOCIDX rows, this CHANGELOG.
- **Status flow:** Phase 5 ✅ DONE → **Phase 6 🔵 ACTIVE (next)** — cutover/hosting, pending Task 0 Cloud Run readiness probe.
...
- **Task 0 version pins resolved** (`package-lock.json` committed): React 19.2.8, Vite 8.2.1, TanStack Router 1.170.21, Tailwind v4.3.3; **Recharts absent** — ChartsRow is an honest empty-state placeholder (no client-side chart contract).
- **Wave 4A — functional shell:** Vite + TanStack Router + Tailwind v4 scaffold in `frontend/` (file-based routes, `routeTree.gen.ts` generated by the Vite plugin, not copied); selective shadcn primitives (button/card/badge/input/label/skeleton/separator/dialog/tooltip/dropdown-menu/scroll-area/sonner); `api.ts` with **snake_case→camelCase boundary normalization** + typed error map; `explorer-store.tsx` per STORE-DRIFT-MATRIX (server-owned state, no seeds, no mock data in prod registry); upload → context → preview → quality → Clear Data wired; MSW + vitest tests; eslint quarantine (no `src/test`/mock imports in production) + no-raw-fetch boundary.
- **Wave 4B — AI UI:** named-SSE reader (`chat-stream.ts`, byte-for-byte Phase 3 wire contract: text/usage/done/error + warning); mounted ChatPanel + AiSummary; streamingId retry rule (no duplicate assistant/user turns — **`ed94679` fixes the typed-error retry path regression**); abort/cancel; per-message keys; usage/ledger display.
- **Playwright Task 9 gate** (`frontend/e2e/test_frontend_flow.py`): load → upload → preview → quality → AI panel → Clear Data → empty state, zero console errors, via the Vite `/api` proxy against real FastAPI.
- **CI:** `frontend-port` job (npm ci → check → build → test → routeTree drift → no-secrets scan) + `frontend-port-e2e` job (uvicorn + vite dev + flow test; `API_SESSION_SECRET` from `${{ secrets.* }}` with `ENVIRONMENT=test` fallback so the credential guard stays clean).
- **Phase 5 spec expanded to ACTIVE:** `migration/specs/phase-5-ga4-drive.md` — Tasks 0–7 (GA4 feasibility + Drive browse-UX research gates; PKCE S256 OAuth with transaction-cookie binding; Drive `download` trust-boundary port of `utils/drive_client.py`; GA4 `pull` with measurement-contract provenance; browse path per D1; React wiring; contract tests + 12-row Drive E2E matrix; guard allowlist for `GA4_*`/`DRIVE_*`). Pending Task 0 research gates + owner decisions D1–D5.
