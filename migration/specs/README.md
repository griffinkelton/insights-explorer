...
>
> **Next spec:** [`phase-5-ga4-drive.md`](phase-5-ga4-drive.md) — **EXPANDED — execution-ready; Task 0 in progress** (decisions D1–D5 settled + implementation details locked; **Gate 1 + Gate 2 documentation-facts evidence recorded 2026-08-06** — SDK pin `google-analytics-data` 0.23.0, quotas verified, Picker API facts; **D4 property probe pending credentials**). GA4 OAuth (PKCE S256, server-owned) + Drive import (port of the hardened `drive_client` trust boundary), contract tests + Drive E2E matrix, React wiring into the Phase 4 shell. Task 0 research gates must close (D4 probe pending) before implementation begins.
