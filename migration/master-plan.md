...
- [ ] **`GET /api/v1/drive/list` contract (if slide-out chosen) — fully specified by the prototype:** params `q` (search) / `folder_id` (browse, default `root`); server-side Drive `files.list` query `trashed = false AND (name contains '<term>' OR '<folder_id>' in parents)`, `pageSize: 50`, `orderBy: folder,modifiedTime desc`, fields `id,name,mimeType,modifiedTime,size,webViewLink,iconLink`; response `{ state, message?, setupHint?, files: [...] }` with states `ready|not_configured|permission|error` (`not_configured` → no credentials; 401/403 → `permission` → reconnect + `drive.readonly`; else `error`). (Archive §4.17; transcript §6.1.)
