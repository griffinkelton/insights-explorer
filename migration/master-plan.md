...
- [ ] **`GET /api/v1/drive/list` contract (if slide-out chosen) — fully specified by the prototype + pagination (2026-08-06; archive §4.18):** params `q` (search) / `folder_id` (browse, default `root`) / **`page_token`** (opaque continuation token, optional); server-side Drive `files.list` query `trashed = false AND (name contains '<term>' OR '<folder_id>' in parents)`, `pageSize: 50`, `orderBy: folder,modifiedTime desc`, fields `id,name,mimeType,modifiedTime,size,webViewLink,iconLink`; response `{ state, message?, setupHint?, files: [...], next_page_token }` — **`next_page_token` is required (opaque token or null); a folder with >50 entries must paginate, not silently truncate**, and the React sheet adds a "Load more" affordance when present. States `ready|not_configured|permission|error` (`not_configured` → no credentials; 401/403 → `permission` → reconnect + `drive.readonly`; else `error`). (Archive §4.17; transcript §6.1.)
...
| Client-supplied file metadata trusted for download | High | Server-side trust boundary: accept `file_id` only, re-fetch metadata, server-side MIME/size enforcement, Sheets export path (Phase 5; archive §4.18) |
