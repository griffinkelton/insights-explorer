...
> **STATUS: ✅ DONE (2026-08-06)** — `f032c9b` on `feat/react-fastapi-migration` (Task 0 gates evidence recorded `ce16045`). GA4 OAuth (PKCE S256, one-time state, transaction-cookie binding, grant-scope verification, session rotation, encrypted server-side tokens) + first pull (5 canonical metrics × `date`, 90 complete days, generic offset pagination proven in mocks, quota record-only + provenance, typed taxonomy incl. non-retryable `ga4_quota_exhausted`); Drive Picker-first (`drive.file`, JIT memory-only picker token `no-store`, `{request_id, file_id}` download trust boundary — metadata re-fetch, declared + actual-byte caps, disk-backed temp, worker-thread, cancellation-safe cleanup, **no-upload boundary**); `oauth_store.py` (10-min TTL transactions); settings + guard allowlist + `.env.example`; React wiring (`ErrorBanner`, `DrivePickerDialog`, `OAuthCallbackPage` + GA4/Drive callback routes, real store members). Evidence: **944 tests** (52 new Phase 5 contract tests), guard exit 0, hooks green; frontend check/build/test green (25 vitest); **both Playwright e2e gates green** (`test_frontend_flow.py` + new `test_phase5_wiring_smoke.py`). **Live Google smoke + post-OAuth property probe explicitly pending D4 sandbox credentials** (`E2E_REAL_GOOGLE=1`, never CI). Full record: `specs/phase-5-ga4-drive.md`.
...
- [ ] `POST /api/v1/ga4/pull` → paginate (`limit`/`offset`, 10k-row pages, ≤9 dimensions, max 250k rows/request) and throttle for the **10 concurrent requests/property (Standard; 50 for 360)** quota; enable `returnPropertyQuota: true` for observability; account for token budgets (200k/day + 40k/hr per property) and the 120 thresholded-requests/hr cap. (Plan amendment 7; archive §3.9, §3.10.) **Pull contract locked (2026-08-06):** five canonical metrics (`sessions`/`totalUsers`/`engagedSessions`/`engagementRate`/`bounceRate`) × `date`, 90 complete days, daily grain ≈ **90 rows** — live pagination won't be exercised; generic offset loop proven via mocked contract tests; quota recorded from successful responses only; `ResourceExhausted` → typed non-retryable `ga4_quota_exhausted` (no retry loop); full taxonomy in `specs/phase-5-ga4-drive.md` Task 3.
...
- [ ] **`POST /api/v1/drive/download` — server-side trust boundary (2026-08-06; archive §4.18):** accept the Drive **`file_id`** only — never trust a client-provided filename, MIME type, or byte size. Server re-fetches file metadata from Drive, enforces `MAX_INGEST_BYTES = 100 MB` (from metadata where available) and the MIME/type allowlist **server-side**, and handles Google-native Sheets via an **export path** (not byte download). Post-download: decompression, row, column, and temp-file lifetime limits; **return the same typed errors as the local upload path**. The React sheet's MIME/name checks are UX guidance only, never the security authority. **Local cross-check (2026-08-06; archive §4.19): this boundary ALREADY EXISTS in `utils/drive_client.py`** (`download_drive_file`) — server-authoritative `files.get(fields="name,mimeType,size")`, `DRIVE_IMPORT_MIME_TYPES` allowlist, Sheets `export_media(mimeType="text/csv")` first-sheet-only, 3-layer size enforcement (metadata preflight → `_BoundedBytesIO` stream cap → final `len()` check), typed `DriveImportError` codes (`unsupported_type/too_large/empty_file/not_found/access_denied/download_failed`). **Phase 5 is a port of this function into `api/services/drive_service.py`, not new design.** Live-verified nuance: Google-native files have **no `size` metadata field** (hence the stream-cap layer); Google imposes a **10 MB export cap** on Sheets/docs exports — so Sheets can never exceed 10 MB regardless of the 100 MB policy; `alt=media` binary downloads have no practical limit (5 TB/file ceiling). **Download hardening + no-upload (2026-08-06):** metadata re-fetch includes `trashed` + `capabilities(canDownload)` (reject both); **declared + actual-byte** enforcement (stream into a disk-backed `NamedTemporaryFile` with sanitized suffix; actual `temp.tell()` cap during transfer); blocking Google client code runs in a worker thread (`anyio.to_thread.run_sync`); temp artifact deleted in `finally`; Google-native files → typed `workspace_export_required` until an export contract is defined (**no automatic export in Phase 5**); **Phase 5 is download-and-ingest only — no upload-to-Drive code** (future export feature = resumable upload, parked). **Canonical download payload `{ request_id, file_id }` (2026-08-06):** `request_id` must match the active server/session picker request (stale/duplicate → typed non-retryable error); only `file_id` carries metadata/download authority; client filename/MIME/size ignored.
- [ ] Port the chosen UI as a **native React component** (not an embedded Streamlit component).
- [ ] `POST /api/v1/drive/picker-token` → returns the OAuth token **and the project number** (`setAppId`) — **only if the Picker iframe (b) is chosen**; document Cloud Resource Manager API enablement; restrict the API key to HTTP referrers. (Plan amendment 2; archive §3.3.) **JIT + memory-only (2026-08-06):** call immediately before Picker opens; return the current valid access token + real `expires_at` when available + `app_id`; `Cache-Control: no-store` + `Pragma: no-cache`; CSRF/origin enforced (POST); browser keeps the token only in component memory and clears it on select/cancel/error; **never revoke on Picker close** (can invalidate the stored grant/refresh relationship).
...
> **STATUS: 🔵 ACTIVE — expanded + deepened (2026-08-06), execution-ready pending Task 0 Cloud Run readiness probe.** Spec `specs/phase-6-cutover-hosting.md` carries the full plan: single-origin FastAPI-serves-Vite, one worker/container, multi-stage Docker, SPA fallback + cache headers, `__Host-` cookies, Redis sessions/locks/OAuth-state migration (single-instance lease lock, no Redlock), Cloud Run deploy, SSE serving, `/healthz` vs `/readyz`, TTL-first cleanup + BackgroundTasks policy, and the §17 operational-readiness deferred gates. **Phase 5 is DONE — this is the next phase.**
...
| 7 | GA4 dim/metric limits (9 dims / 10 metrics, 7 for funnel) | Phase 5 | Reported, not live-verified (limits page 404s) — re-verify in Task 0. **Pull contract locked (2026-08-06):** five canonical metrics × `date`, 90 days, daily grain; generic offset pagination proven via mocks; `returnPropertyQuota=True` recorded from successful responses only; typed error taxonomy — `ga4_quota_exhausted` non-retryable, no retry loop; connection/reconnect split (`ga4_connection_required` 409 · `ga4_reconnect_required` 401) (`specs/phase-5-ga4-drive.md` Task 3). **Task 0 evidence (2026-08-06):** SDK pin `google-analytics-data` 0.23.0 (async shape verified by pinned-SDK probe); official quotas confirmed 200k/day + 40k/hr (Standard) · 10 concurrent · 120 thresholded/hr; max 250,000 rows/request, default 10,000; 9-dim cap confirmed; property probe pending D4 |
