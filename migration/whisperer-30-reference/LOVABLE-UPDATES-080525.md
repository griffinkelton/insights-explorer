...
- **`measurement-contract.ts` (110 lines)** — a **faithful TS transcription of the canonical `plans/ga4-measurement-contract.md`**: all 5 metric IDs match (`daily_reach`, `page_device_engagement_rate`, `questionnaire_start_count`, `questionnaire_completion_rate`, `post_questionnaire_action_rate`), statuses match (provisional ×2, unavailable ×3), and numerator/denominator/grain/eventMapping/blockedBy/limitations match row-for-row. Adds helpers `computableMetrics()` (filters `unavailable`) and `contractContext()` (the governance RULE string fed to the model). **Cross-checked 2026-08-06 against the canonical doc — verified faithful.**
