...
6. **Pagination gap (2026-08-06):** the prototype's list has `pageSize: 50` but **no continuation token** — folders >50 entries silently truncate. The port adds `page_token` (request) + `next_page_token` (response, opaque or null) + a "Load more" affordance in the sheet.
7. **Download trust boundary (2026-08-06):** the sheet's MIME/name checks are UX guidance only, never the security authority. `POST /api/v1/drive/download` must accept `file_id` only, re-fetch metadata server-side, enforce 100 MB + MIME allowlist server-side, use an export path for Google-native Sheets, apply post-download decompression/row/column/temp-file limits, and return the same typed errors as upload.
