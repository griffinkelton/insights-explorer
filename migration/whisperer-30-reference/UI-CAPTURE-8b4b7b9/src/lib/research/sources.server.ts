...
import { EVIDENCE_SOURCE, evidenceContext, lastSync } from "@/lib/evidence/mock-evidence";
import { contractContext } from "@/lib/measurement-contract";
import { insightContext } from "@/lib/insights/engine";
...
const ga4Source: ResearchSource = {
  ...
  load: async () => {
    ...
    return {
      ...
      context: [
        `GA4 property (measurement ID ${id}) — behavioural and demographic metrics:`,
        buildDataContext(),
        "",
        contractContext(),
      ].join("\n"),
...
const evidenceSource: ResearchSource = {
  id: "evidence",
  ...
  check: () =>
    status(
      ...
      "Evidence dashboard",
      "ready",
      `Synced ${lastSync.records.filter((r) => r.outcome === "synced").length} allowlisted datasets.`,
    ),
  load: async () => ({
    status: evidenceSource.check(),
    context: [evidenceContext(), "", insightContext()].join("\n"),
    links: [{ label: EVIDENCE_SOURCE.label, url: EVIDENCE_SOURCE.baseUrl }],
  }),
};

// Add a new source (GitHub, Notion, …) by appending an adapter here.
export const researchSources: ResearchSource[] = [ga4Source, driveSource, evidenceSource];
