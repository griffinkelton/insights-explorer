...
| [migration/README.md](migration/README.md) | React/FastAPI migration — index of all planning docs, research, and reference capture | Before starting migration work (Phases 1–5 implemented; Phase 6 = cutover/hosting next) |
...
> Moving the product from Streamlit to a React frontend (whisperer-30 components) + FastAPI backend over the existing `utils/` layer. **Phases 1–5 ✅ complete on `feat/react-fastapi-migration`** (944 Python tests, guard exit 0; frontend check/build/test green — 25 vitest; both Playwright e2e gates green). Phase 5 (GA4/Drive, `f032c9b`) shipped the full vertical slice: server-owned GA4 OAuth (PKCE S256, one-time state, txn-cookie, session rotation, encrypted tokens) + first pull (5 canonical metrics × `date`, pagination proven in mocks, quota + provenance + typed taxonomy); Drive Picker-first (`drive.file`, JIT memory-only picker token, `{request_id, file_id}` download trust boundary, disk-backed temp, no-upload); React wiring (`ErrorBanner`, `DrivePickerDialog`, GA4/Drive callback routes) + 52 contract tests. **Live Google smoke explicitly pending D4 sandbox credentials** (never falsely closed). Phase 6 (cutover/hosting) is the next phase — expanded + deepened, pending Task 0 Cloud Run readiness probe. Entry gates closed: credentials remediated · branch + Streamlit freeze active · retention/AI-boundary policy approved.** Start at `migration/README.md`.

| File | Purpose (why read this?) | What it covers | Status |
|---|---|---|---|
| [migration/README.md](migration/README.md) | Entry point — index of the migration docs (bucket-organized: policies/ · specs/ · archive/) and how they relate | Decision summary, bucket-organized doc tables, dependency diagram, addenda system, reading paths, pre-implementation action items | 🔵 Phases 1–5 done (2026-08-06) |
| [migration/master-plan.md](migration/master-plan.md) | **The execution coordinator — START IMPLEMENTATION HERE** | Phases 0–6 with inputs/tasks/exit criteria, 5 cross-cutting workstreams (state, contract, tests, security, CI/CD), target repo file layout, critical path, open decisions, risk register, doc→phase source map | 🔵 Plan — Phases 1–5 done; Phase 6 next (2026-08-06) |
...
| [migration/specs/phase-3-ai-analysis.md](migration/specs/phase-3-ai-analysis.md) | **DONE — AI/analysis spec** (Gemini/chat SSE + analysis; research gate run, 13 decisions confirmed/refined, verification notes) | ✅ Done (2026-08-06) — `bb6f564` on `feat/react-fastapi-migration`, 859 tests, guard exit 0 |
| [migration/specs/phase-4-react-port.md](migration/specs/phase-4-react-port.md) | **DONE — React port spec**, absorbs F3; Task 0–9 (Vite-proxy dev topology, selective shadcn, drift-matrix store wiring, named-SSE reader) | ✅ Done (2026-08-06) — `075dfa4` + `ed94679`, 17 vitest tests, Playwright Task 9 gate green |
| [migration/specs/phase-5-ga4-drive.md](migration/specs/phase-5-ga4-drive.md) | **DONE — GA4/Drive spec**, absorbs F4 GA4 sections; Tasks 0–7 (PKCE OAuth, Drive trust-boundary port, GA4 pull + provenance, Drive E2E matrix) | ✅ Done (2026-08-06) — `f032c9b`, 944 tests, both e2e gates green; live Google smoke pending D4 credentials |
| [migration/specs/phase-6-cutover-hosting.md](migration/specs/phase-6-cutover-hosting.md) | Phase 6 spec — cutover + Cloud Run + §17 deferred gates; Task 0–10 execution-ready (single-origin FastAPI-serves-Vite, one worker/container, multi-stage Docker, `__Host-` cookies, Redis sessions/locks, SSE serving) | 🔵 Active — expanded + deepened, next phase (2026-08-06) |
| [migration/specs/freebuff-prompt-wire-react-store.md](migration/specs/freebuff-prompt-wire-react-store.md) | F3 — frontend wiring prompt (**SUPERSEDED FOR EXECUTION** 2026-08-06; feeds `phase-4-react-port.md` via STORE-DRIFT-MATRIX) | 🟡 Superseded — reference |
| [migration/specs/phase-1-api-react-callback-tests-implementation.md](migration/specs/phase-1-api-react-callback-tests-implementation.md) | F4 — Phase 1 implementation packet (**SUPERSEDED FOR EXECUTION** 2026-08-06; embedded in `phase-1-upload-slice.md`, GA4 sections → `phase-5-ga4-drive.md`) | 🟡 Superseded — reference |
...
| [migration/specs/phase-3-ai-analysis.md](migration/specs/phase-3-ai-analysis.md) | AI/analysis spec — DONE (SSE chat, ledger, ai_service, token guard) | ✅ Done | 2026-08-06 |
| [migration/specs/phase-4-react-port.md](migration/specs/phase-4-react-port.md) | React-port spec — DONE (Waves 4A+4B, MSW+vitest, Playwright gate, CI jobs) | ✅ Done | 2026-08-06 |
| [migration/specs/phase-5-ga4-drive.md](migration/specs/phase-5-ga4-drive.md) | GA4/Drive spec — ACTIVE (Tasks 0–7, pending research gates; D1–D5 settled) | 🔵 Execution-ready | 2026-08-06 |
...
*This index was last updated 2026-08-06 — migration docs: **Phases 1–4 implemented on `feat/react-fastapi-migration`** (859 Python tests, guard exit 0): Phase 3 (AI/analysis) `bb6f564` — SSE chat, summary/forecast/funnel, usage ledger, ai_service, async aio streaming, Task 0 countTokens probe (google-genai 2.14.0); Phase 4 (React port) `075dfa4` + `ed94679` — Waves 4A+4B (Vite/TanStack/Tailwind scaffold, boundary-normalized api.ts, drift-matrix store, named-SSE reader, mounted Chat/AiSummary, 17 vitest/MSW tests, Playwright Task 9 gate, CI frontend jobs). Phase 5 (GA4/Drive) + Phase 6 (cutover/hosting) expanded to ACTIVE with owner guidance — Phase 5: Tasks 0–7 (PKCE OAuth, Drive trust-boundary port, GA4 pull with contract provenance, Drive E2E matrix) pending research gates (decisions D1–D5 settled); Phase 6: single-origin FastAPI-serves-Vite, one worker/container, multi-stage Docker, `__Host-` cookies, Redis sessions/locks, Cloud Run deploy, SSE serving. Prior update 2026-08-05 — added the `migration/` folder: React/FastAPI migration docs (compiled archive, 6-phase plan, **master execution plan**, F3/F4 prompts, GLM comparison, session-state inventory, Docker pattern, .env rotation checklist, branch/freeze policy, test-layer inventory, data-retention policy, master-plan revision). Prior update 2026-08-02 — v0.3.0 spec: Phase 2 complete (all 5 steps, 664 tests), Phase 3.0–3.1 complete (Picker wiring + UX polish, 56 tests); Phase 3.2 (Playwright) + 3.3 (browser matrix) + 4 (docs) pending. ga4-insights-sketch v3 (662 lines, 5-gate phasing).*
