...
| [plans/00-sprints/🔵 v0.3.0-drive-import-spec.md](plans/00-sprints/🔵%20v0.3.0-drive-import-spec.md) | Thorough 5-phase implementation spec: Phase 0 transport proof (✅ complete) → Phase 1 security hardening (✅ complete) → provenance → picker UI → docs | 12 design decisions, file-level precision with code sketches, declared bidirectional component as the chosen transport (runtime args, `setComponentValue()` → `{kind, requestId, fileId}`), server metadata authority, bounded download + 3-layer size validation, atomic import, per-phase acceptance criteria | ✅ Phase 1 complete (631 tests); Phases 2–4 pending |
...
| [plans/audit/✅ v0.3.0-phase-1-closeout.md](plans/audit/✅%20v0.3.0-phase-1-closeout.md) | Phase 1 closeout audit — server-side download client + component validation + CI gate evidence | `download_drive_file()` (3-layer size validation, server metadata authority, MIME allowlist, typed errors), `PickerSelection` wrapper, token-safety source scans, `TestDriveScopeRestricted`, frontend CI gate (`npm ci` → typecheck → build → bundle assertion), all 10 acceptance criteria ✅ | ✅ Complete (631 tests) — commit `9754189` |
...
| [plans/00-sprints/🔵 v0.3.0-drive-import-spec.md](plans/00-sprints/🔵%20v0.3.0-drive-import-spec.md) | 5-phase implementation spec — Phase 0 ✅ complete (Option B declared component selected), Phase 1 ✅ complete, security, provenance, picker UI, docs | ✅ Phase 1 complete (631 tests) | 2026-08-01 |
| [plans/audit/✅ v0.3.0-phase-1-closeout.md](plans/audit/✅%20v0.3.0-phase-1-closeout.md) | Phase 1 closeout — download client, wrapper validation, token-safety tests, CI frontend gate | ✅ Complete (631 tests) — commit `9754189` | 2026-08-01 |
...
*This index was last updated 2026-08-01 — v0.3.0 Phase 1 closeout (`phase-1-closeout.md`) added and v0.3.0 rows advanced to Phase 1 complete (631 tests, commit `9754189`); Phase 0 decision record (`phase-0-debug-summary.md`) indexed; Option B declared component is the chosen transport, not a fallback; IDEAS #29 credential rotation closed (key rotated, GA4 re-authed with `drive.file`, repo sweep clean, credential-leak guard added — see IDEAS.md, CHANGELOG, `14fd6b9`).*
